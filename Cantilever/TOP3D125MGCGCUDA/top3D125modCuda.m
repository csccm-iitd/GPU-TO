...
filename = strcat('3D125mgcgmodCUDA_',num2str(nl),'_', num2str(nelx),'_', num2str(nely),'_', num2str(nelz),'caml','.txt');    
...
while ch > 1e-6 && loop < maxit
  ...
  cgtol=1e-9;
  cgmax=200;
  [cgres,cgiters]=mgcg1(Ke0,F1,U,CX,fixed',nfd,nelx,nely,nelz,nl,cgtol,cgmax);
