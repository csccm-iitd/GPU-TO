...
filename = strcat('3D125_',num2str(nl),'_', num2str(nelx),'_', num2str(nely),'_', num2str(nelz),'caml','.txt');    
fileID = fopen(filename,'w');
fprintf(fileID, ['Top3D125 \n\nnl:%2d, nelx:%4d, nely:%4d, nelz:%4d, '...
...
top3D125_f(64,32,32,0.12,3, sqrt(3)*2, 3, 'N', 0.4,2, 0.2, 100, filename);


function top3D125_f(nelx,nely,nelz,volfrac,penal,rmin,ft,ftBC,eta,beta,move,maxit, filename)
...
while ch > 1e-6 && loop < maxit
  ...
  fprintf( fileID, 'It.:%5i C:%6.5e Time:%4.3f V:%7.3f ch.:%0.2e penal:%7.2f beta:%7.1f eta:%7.2f lm:%0.2e MemUsed:%6.4e \n', ...
            loop, F'*U, t, mean(xPhys(:)), ch, penal, beta, eta, lmid,  user.MemUsedMATLAB );
  fclose(fileID);
