...
filename = strcat('3D125mod_',num2str(nl),'_', num2str(nelx),'_', num2str(nely),'_', num2str(nelz),'caml','.txt');    
...
while ch > 1e-6 && loop < maxit
  ...
  WRK=zeros(100*nDof,1);
  nfd=length(fixed);
  cgtol=1e-9;
  cgmax=200;
  [cgres,cgiters]=mgcg1(Ke0,F1,U,CX,fixed',WRK,nfd,nelx,nely,nelz,nl,cgtol,cgmax);
...
% isovals = shiftdim( reshape( xPhys, nely, nelz, nelx ), 2 );
isovals = shiftdim( reshape( xPhys, nely, nelz, nelx ), 2 );
% isovals = reshape( xPhys, nelx, nely, nelz );
isovals = smooth3( isovals, 'box', 1 );
patch(isosurface(isovals, .5),'FaceColor','b','EdgeColor','none');
patch(isocaps(isovals, .5),'FaceColor','r','EdgeColor','none');
